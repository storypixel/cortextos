...
> **Runtime:** This is an `opencode` agent. Telegram replies go through `cortextos bus send-telegram <chat_id> '<msg>'`. The OpenCode terminal UI is not user-visible — every user-facing message MUST use this command.
...
The SOUL.md template (`${CTX_FRAMEWORK_ROOT}/templates/agent-opencode/SOUL.md`) contains all 7 operational pillars. You MUST preserve every section when writing. Update only:
...
```bash
TEMPLATE=$(cat "${CTX_FRAMEWORK_ROOT}/templates/agent-opencode/SOUL.md")
...
9. **Ask for workflows:**
   ...
   - If the workflow is complex (multi-step procedure), create a skill file at `plugins/cortextos-agent-skills/skills/<workflow-name>/SKILL.md` with YAML frontmatter and detailed steps, and a corresponding symlink at `~/.config/opencode/skills/${CTX_AGENT_NAME}__<workflow-name>` pointing to that directory.
...
11b. **Ask about migration:**
    ...
    **END YOUR TURN.** If the user says no migration, skip to step 12. If yes, continue below.
    ...
    - Copy any custom skills: for each skill in `<old_dir>/plugins/cortextos-agent-skills/skills/` (or `<old_dir>/.claude/skills/` if migrating from a Claude-Code agent), offer to copy it into `${CTX_AGENT_DIR}/plugins/cortextos-agent-skills/skills/` and create a `~/.config/opencode/skills/${CTX_AGENT_NAME}__<skill_name>` symlink for each
...
Write to `${CTX_AGENT_DIR}/SYSTEM.md` with org name, runtime (`opencode`), timezone, orchestrator, dashboard URL, and team roster from Step 8.
...
if [ "$TOOLS_LINES" -lt 100 ]; then
  cp "${CTX_FRAMEWORK_ROOT}/templates/agent-opencode/TOOLS.md" "${CTX_AGENT_DIR}/TOOLS.md"
fi
```
...
if [ -n "$MISSING" ]; then
  ...
  if echo "$MISSING" | grep -q "TOOLS.md"; then
    cp "${CTX_FRAMEWORK_ROOT}/templates/agent-opencode/TOOLS.md" "${CTX_AGENT_DIR}/TOOLS.md" 2>/dev/null
  fi
else
